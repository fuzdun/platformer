...
init_render_buffers :: proc(gs: ^Game_State, rs: ^Render_State) {
    for shader in ProgramName {
        rs.shader_queues[shader] = make(map[string][dynamic]glm.mat4)
        for shape in SHAPE_NAMES {
            rs.shader_queues[shader][shape] = make([dynamic]glm.mat4)
        }
    }
    for shader in ProgramName {
...
init_level_render_data :: proc(gs: ^Game_State, shst: ^ShaderState, rs: ^Render_State) {
    ...
    for lg in gs.level_geometry {
        append(&rs.static_transforms, trans_to_mat4(lg.transform))
    }
...
Shader_Queues :: [ProgramName]map[string][dynamic]glm.mat4
...
draw_triangles :: proc(gs: ^Game_State, rs: ^Render_State, shst: ^ShaderState, ps: ^Physics_State, time: f64) {
    ...
    for lg, lg_idx in gs.level_geometry {
        ...
        for shader in lg.shaders {
            append(&rs.shader_queues[shader][lg.shape], rs.static_transforms[lg_idx])
        }
    }
    ...
    for queue, shader in rs.shader_queues {
        ...
        for shape, transforms in queue {
            if len(transforms) == 0 do continue
            ...
            append(&render_queue.transforms, ..transforms[:])
    ...
    append(&player_rq.commands, gl.DrawElementsIndirectCommand{
        u32(len(gs.player_geometry.indices)), 1, 0, 0, 1
    })
    ...
    // draw elements from render queues
    //for queue, shader in rs.shader_render_queues {
