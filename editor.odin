...
get_selected_geometry_dists :: proc(es: ^Editor_State, ps: Physics_State, lgs: Level_Geometry_Soa) {
...
editor_move_object :: proc(lgs: ^Level_Geometry_State, lrs: Level_Resources, es: ^Editor_State, is: Input_State, ps: ^Physics_State, rs: ^Render_State, delta_time: f32) {
    ...
    if is.q_pressed && es.can_add {
        ...
        add_geometry(lgs, lrs, ps, rs, es, new_lg)
    }
    ...
    if is.bck_pressed && es.can_delete {
        remove_geometry(lgs, lrs, ps, rs, es)
    }
