...
narrow_phase_collisions :: proc(gs: ^Game_State, ps: ^Physics_State) {
    ...
    for po in ps.objects {
        ...
        for i := 0; i <= l - 3; i += 3 {
            ...
            closest_pt := closest_triangle_pt(tri_vertex0, tri_vertex1, tri_vertex2, ppos32)
            if la.length2(closest_pt - ppos32) < player_sq_radius {
                if gs.input_state.c_pressed {
                    ...
                    append(&ps.debug_render_queue.indices[.BlueOutline], off, off + 1, off + 1, off + 2, off + 2, off)
                    append(&ps.debug_render_queue.vertices, v0, v1, v2)
                }
            }
        }         
    }
